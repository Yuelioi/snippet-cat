...
export async function downloadDepsFiles(
    ...
    syncFolder.forEach(async function (syncFileInfo: any) {
        ...
        if (syncFileInfo.type !== 'directory') {
            const syncStatus = <any>await client.stat(syncFullPath);
            ...
            if (localModTime !== syncModTime) {
                const content = <string>(
                    await client.getFileContents(syncFullPath, { format: 'text' })
                );
...
export async function uploadDepsFiles(
    localFolderPath: string,
    syncFolderPath: string,
    client: any
) {
